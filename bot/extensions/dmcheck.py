...
class DMCheck(commands.Cog):
    ...
    @commands.command(name="dmcheck", pass_context=True, guild_ids=guild_ids)
            ...
            for i in range(len(users)):
                ...
                if user := self.bot.get_user(int(users[i])):
                    try:
                        ...
                        await ctx.send(
                            content=f"Could not open <@{users[i]}>'s dms.",
                            allowed_mentions=[],
                        )
                        continue
                else:
                    await ctx.send(
                        content=f"Failed to grab user <@{users[i]}>",
                        allowed_mentions=[],
                    )
            user_message_count = dict(
                sorted(user_message_count.items(), key=lambda x: x[1], reverse=True)
            )
            embed = Embed(title="DM Check", color=Color.blue())
            for user, messages in user_message_count.items():
                if messages:
                    embed.add_field(
                        name=user.display_name, value=messages, inline=False
                    )
            await ctx.send(embed=embed)
        else:
            await ctx.send(
                "No users to check, therefore ending. Remember to onboard with the init command."
            )
